...
核查日期：2026-09-09。正文、教学程序与图像复审已完成；远端提交和 GitHub 页面验证仍按实际进度记录。此清单保留原始目标，不以文件数量替代内容验收。

| 原始要求 | 应有的直接证据 | 当前状态 |
| --- | --- | --- |
| GitHub 新建 public 仓库 | [公开仓库](https://github.com/Shiba-hua/token-efficient-llm)；推送后核对远端提交与 CI | 仓库已创建，内容推送待验证 |
...
| 技术推导与可运行代码 | [七个程序实际输出](example-runs.json)；六项评分单元测试、后训练算术自检通过 | 本地完成，远端 CI 待验证 |
...
| 原生文本表格 | README、章节和来源记录使用 Markdown 文本表格 | 源码完成，GitHub 渲染待验证 |
...
| 初学者可快速学习 | 三条阅读路线、[术语表](../docs/glossary.md)、逐章练习、标准库 CPU 算例与完整图位检查 | 教学结构完成，GitHub 页面检查待完成 |
