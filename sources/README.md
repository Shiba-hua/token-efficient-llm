...
`reference/` 文件名采用本章方法名或可识别的短标题。论文身份仍由 `paper_id`、arXiv/DOI 和版本维护；`method_name` 与 `reference_filename` 负责阅读时的名称，`artifact` 是唯一当前入口。改名映射与正文哈希见[记录](../meta/reference-renames.json)。
