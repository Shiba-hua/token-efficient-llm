...
\#Token_I^{TAO}
&=\Theta(C+\mathbf S+Q)
 +\sum_{j=1}^{k-1}\Theta\left(C+\mathbf S+Q+\sum_{t=1}^{j}(T_t+A_t+O_t)\right)\\
&=k\Theta(Q)+k\Theta(C)+k\Theta(\mathbf S)
 +\sum_{j=1}^{k-1}(k-j)\Theta(T_j+A_j+O_j).
\end{aligned}
```

前 $`k`$ 次调用重复发送 $`Q,C,\mathbf S`$；第 1 步的 TAO 会被之后的 $`k-1`$ 次请求再次发送，第 2 步会被之后的 $`k-2`$ 次发送。因此，若每步轨迹长度近似相同，历史轨迹项随 $`k`$ 呈二次累积；固定上下文、示例和问题项至少呈 $`k`$ 次累积。这里讨论的是 LLM 输入 token；工具内部若也调用 LLM，实际总量还会增加。
...
\begin{aligned}
\#Token_I^{ReWOO}
&=\Theta(C_{planner}+\mathbf S+Q)
 +\Theta(C_{solver}+Q+\sum_{j=1}^kP_j+E_j)\\
&\approx 2\Theta(Q)+2\Theta(C)+\Theta(\mathbf S)
 +\sum_{j=1}^k\Theta(P_j+E_j).
\end{aligned}
```

因此问题和上下文只分别进入 Planner、Solver 两次，示例只给 Planner 一次；历史 TAO 不会逐步堆叠。若把 TAO 数和 PE（Plan–Evidence）数人为对齐，作者说相对 TAO 方案，节省量对 $`Q,C,\mathbf S`$ 是线性的、对每步 $`T,A,O`$ 的累积项随 $`k`$ 呈二次差异。这个推导只说明输入 prompt 的重复结构；它没有推出总墙钟延迟、工具服务成本或 Solver/Planner 输出 token 一定更少。作者实验把 LLM-based tool 的 token 也计入总 token，但公式本身明确忽略了工具内部复杂度。
...
![Observation-dependent TAO prompting versus ReWOO Plan-Work-Solve prompting.](../../../assets/papers/2305.18323/figure2-tao-vs-pws.png)
...
![Performance degraded when incrementally adding extraneous tools.](../../../assets/papers/2305.18323/figure-extra-tools.png)
...
![Offloading foreseeable reasoning from GPT-3.5 into Alpaca 7B and Planner 7B.](../../../assets/papers/2305.18323/figure3-specialization.png)

**图 3（`Figure3.pdf`）读法。** 左侧是 GPT-3.5 产生的计划数据，中间先得到 Alpaca 7B，右侧再用正确的计划数据得到 Planner 7B。监督目标是规划行为而非工具返回内容，所以工具仍可在部署时替换。

![Performance effect of changing the LLM in ReWOO; Planner 7B tuned on HotpotQA and TriviaQA.](../../../assets/papers/2305.18323/figure-finetuning.png)
