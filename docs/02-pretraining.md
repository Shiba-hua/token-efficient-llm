...
令原始文本为 $s$，固定 tokenizer 为 $T$，编码序列为 $z_{1:L}=T(s)$。$z_t$ 是词表 $V$ 中的离散编号，$L>0$ 为序列长度。为简化表述，序列包含结束标记；初始前缀由起始标记提供。概率链式法则给出：

$$
p_\theta(z_{1:L})=\prod_{t=1}^{L}p_\theta(z_t\mid z_{<t}).
$$

这是按顺序展开联合概率的恒等关系。模型的近似在于用有限参数和有限上下文表达每个条件概率，而不是链式法则本身需要相邻 token 独立。

取负对数，把乘积变为求和。对包含 $N_{\rm tok}>0$ 个有效目标 token 的语料，常用逐 token 平均负对数似然：

$$
\mathcal L_{\rm NTP}(\theta)
=-\frac{1}{N_{\rm tok}}\sum_{i}\sum_{t=1}^{L_i}
\log p_\theta(z_{i,t}\mid z_{i,<t}).
$$
...
\quad
Z=\sum_t\sum_{k=1}^{K}\mathbf1[t+k\le L]\lambda_k>0.
$$
...
第二个实验使用二元模型，把完整前缀近似为前一个 token。对计数 $n(u,v)$ 与平滑系数 $\alpha>0$，预测为：
