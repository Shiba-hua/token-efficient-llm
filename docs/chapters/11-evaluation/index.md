...
![本章研究路线图](../../../assets/maps-v3/11-evaluation.png)
...
原有基准、真实样例和判分细节保留为[评测技术专题](../../technical/09-evaluation.md)。来源与版本差异见[基准索引](../../../sources/evaluation-map.md)，这些基准解决不同问题，不应拼成单一排行榜。
...
[查看原图，可放大](../../../assets/papers-v3/2407.01502/pareto-hotpot.png) · [论文来源](https://arxiv.org/abs/2407.01502v1)
...
当 $F_A>F_B$ 且 $v_A<v_B$，A需要足够使用次数才回本。这个线性模型要求相近任务、调用行为和单位成本；数据分布或价格变化后要重估。小团队尤其需要知道一次性教师生成、调参和训练能否被未来部署摊销。[精读](reference/2407.01502.md)
...
[查看原图，可放大](../../../assets/papers-v3/2412.21187/figure-1-token-accuracy-part-1.pdf) · [论文来源](https://arxiv.org/abs/2412.21187v2)

![Illustration of overthinking: 2+3 token usage and MATH500 token-accuracy plot](../../../assets/papers-v3/2412.21187/figure-1-token-accuracy-part-2.png)

[查看原图，可放大](../../../assets/papers-v3/2412.21187/figure-1-token-accuracy-part-2.pdf) · [论文来源](https://arxiv.org/abs/2412.21187v2)

原文的模型/任务比较显示冗余与准确率收益可以并存，困难题上的反思仍可能有用。首次正确位置分布只在存在正确解的响应中归一化，第 $m$ 个解法的新颖性只在拥有第 $m$ 段的响应中计算，不能外推为全部测试题的成功概率。外部分段与聚类模型的误差和成本也需要考虑。[精读](reference/2412.21187.md)

## 可解释性如何支持机制判断

注意力热图、token置信度、latent词表投影和案例都能提出机制假设，却有各自的选择条件。例如只在答对题上查看中间结果命中率，不能推出所有题上都拥有相同可解释性；低注意力也不证明某位置可被独立删掉。要支持因果解释，应通过受控消融或干预区分替代原因，同时保留性能与成本。[DeepConf](../06-reasoning/reference/2508.15260.md) · [CODI](../04-architecture/reference/2502.21074.md)
