...
![Fig.1：ReAct与LLMCompiler在HotpotQA示例中的运行时对比](../../../../assets/papers-v3/2312.04511/fig-teaser.png) 对应作者 Fig. 1，资产 [`assets/teaser_fix.pdf`](../../../../assets/papers-v3/2312.04511/fig-teaser.pdf)。左侧 ReAct 为 search Scott Derrickson→把观察重新放回 LLM→search Ed Wood→回答，右侧 Planner 一次生成两项 search，Executor 并行后回答；示例报告 1.8× HotpotQA latency speedup。读图时应注意这只是 2-way 说明图，不能代表所有任务的并行宽度。

![Fig.2：Planner、Task Fetching Unit、Executor与DAG依赖替换概览](../../../../assets/papers-v3/2312.04511/fig-overview.png) 对应作者 Fig. 2，资产 [`assets/overview_fix.pdf`](../../../../assets/papers-v3/2312.04511/fig-overview.pdf)。图中 `$1/$2` 是并行搜索，`$3` 是依赖两者的 math，`$4` 是最终 LLM；TFU 的队列和 dependency resolution 把占位符替换为真实结果。这张图是理解三组件信息流的主证据。

![Fig.3：独立、分层依赖和汇合型函数调用图样例](../../../../assets/papers-v3/2312.04511/fig-dependency-patterns.png) 对应作者 Fig. 3，资产 [`assets/dependencies_new_3.pdf`](../../../../assets/papers-v3/2312.04511/fig-dependency-patterns.pdf)。图 (a) 是各自独立的 analyzer；(b) 是多个搜索→数学汇总→比较；(c) 是两组州级搜索→州内求和→比较。它把“并行宽度”和“依赖深度”放在同一张图里，支撑 HotpotQA/Movie 与 ParallelQA 的实验分类。

![Fig.A.1：Movie Recommendation中ReAct、ReAct†、LLMCompiler函数调用次数分布](../../../../assets/papers-v3/2312.04511/fig-movie-failure.png)、![Fig.A.2：按ReAct调用次数分组的Movie Recommendation准确率](../../../../assets/papers-v3/2312.04511/fig-movie-accuracy.png)、![Fig.A.3：HotpotQA中ReAct与LLMCompiler函数调用次数分布](../../../../assets/papers-v3/2312.04511/fig-hotpot-failure.png)、![Fig.A.4：按ReAct调用次数分组的HotpotQA准确率](../../../../assets/papers-v3/2312.04511/fig-hotpot-accuracy.png) 分别对应附录 Fig. A.1–A.4，资产是 `movie_dist.pdf`、`movie_acc.pdf`、`hotpot_dist.pdf`、`hotpot_acc.pdf`。它们不是新的主 benchmark，而是解释 ReAct† 为什么必要：Movie 约 85% ReAct 样例提前结束，LLMCompiler 约 99% 完成 8 次搜索；HotpotQA LLaMA ReAct 约 10% 超过 4 次重复调用。作者还指出某些三次调用的 ReAct 例子通过替代实体名搜索能成功，这是 ReAct 适应性的少量反例（少于 3%），不应省略。

![Fig.E.5：ParallelQA按最大可并行任务数分组的延迟](../../../../assets/papers-v3/2312.04511/fig-parallelqa-scaling.png) 对应附录 Fig. E.5，资产 [`assets/custom.pdf`](../../../../assets/papers-v3/2312.04511/fig-parallelqa-scaling.pdf)，给出 ParallelQA 按最大并行任务数分组的延迟，展示 ReAct 近似线性增长和 LLMCompiler 较缓增长。![Fig.J.6：Game of 24的Tree-of-Thoughts搜索树与top-5保留](../../../../assets/papers-v3/2312.04511/fig-tot.png) 对应附录 Fig. J.6，资产 [`assets/tot.pdf`](../../../../assets/papers-v3/2312.04511/fig-tot.pdf)，展示 Game of 24 的 ToT 搜索树：节点是候选状态，边是单步运算，评估后保留 top-5；它帮助理解为何每轮需要动态重规划。
