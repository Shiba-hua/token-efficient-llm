...
![Performance and efficiency scaling](../../../../assets/papers-v3/2506.15841/fig-scaling.png) 原图 assets/main_result_fig_high_quality.pdf（正文 Fig.4）：横轴 objective，纵轴为 EM count 和 peak token。只有平缓 peak 与仍上升 EM 一起支持 scalability。
...
![Emergent internal-state behaviors](../../../../assets/papers-v3/2506.15841/fig-behavior.jpg) 原图 assets/mem1_behavior_v2.jpg（Fig.5）展示：分别保存多个问题；卡住时转移；把检索事实写入下一查询；新证据到来时更新；自我验证；规划分解；迭代搜索；查询过窄时扩大。是定性轨迹，没有行为发生率。
