...
\operatorname{mean}(F_{i f+j,:},\operatorname{axis}=1).
```

再以帧级分数做 softmax，得到帧的预算分布 $`p`$，并为每帧分配预算 $`n_i=\lfloor Np_i\rfloor`$；在该帧内部按照 token 的平均相似度取 top-$`n_i`$：

```math
p=\operatorname{softmax}(\hat F),\qquad
\operatorname{indices}_{i}=\operatorname{topk}
\left(\operatorname{mean}(F_{if:(i+1)f,:},\operatorname{axis}=1),n_i\right).
```

保留的第一阶段序列为 $`\mathbf S_{p1}=\mathbf S[\cup_i\operatorname{indices}_i]`$。直观例子：查询是“会议改到周五几点？”，若某一秒里出现“Friday at 3 pm”，该帧的多个 token 会与 query 的 “Friday/time” 更相似，帧预算增加，帧内 top-k 更可能保留连续语义单元；与之相邻的寒暄帧得分低，预算变少。这个例子是对公式的教学演示，论文没有公布某个样本的真实相似度矩阵。
...
```math
(W_Q^b,W_K^b,S^b)=\operatorname{sign}(W_Q,W_K,S_{p1}).
```
...
```math
A=\operatorname{softmax}\left(\frac{Q'K'^\top}{\sqrt{d_k}}\right).
```

对每个候选 token 汇总其平均注意力（式（12）写成 $`\operatorname{mean}(A,\operatorname{axis}=1)`$），取 top-$`k`$ 得到最终语音序列：

```math
S_{p2}=S_{p1}[\operatorname{topk}(\operatorname{mean}(A,\operatorname{axis}=1),k)].
```
...
![The proposed SpeechPrune, with two phases of token pruning.](../../../../assets/papers-v3/2412.12009/fig1-method.png)
...
![Qualitative analysis of token embeddings via t-SNE visualization: SpeechPrune versus random pruning.](../../../../assets/papers-v3/2412.12009/fig2-qualitative-analysis.png)
...
![Ablation study comparing first phase only, second phase only, and SpeechPrune on SPIRAL-H.](../../../../assets/papers-v3/2412.12009/fig3-ablation-spiral-h.png)
...
- ![The proposed SpeechPrune, with two phases of token pruning.](../../../../assets/papers-v3/2412.12009/fig1-method.png)：原 Fig. 1，方法总览，LaTeX `secs/introduction.tex`，资产 `assets/fig1-method.pdf`；作者源图，未裁剪，CC BY 4.0 元数据，允许署名复用。
- ![Qualitative analysis of token embeddings via t-SNE visualization: SpeechPrune versus random pruning.](../../../../assets/papers-v3/2412.12009/fig2-qualitative-analysis.png)：原 Fig. 2，t-SNE 定性比较，LaTeX `secs/experiment.tex`，资产 `assets/fig2-qualitative-analysis.pdf`；作者源图，未裁剪，CC BY 4.0 元数据，允许署名复用。
- ![Ablation study comparing first phase only, second phase only, and SpeechPrune on SPIRAL-H.](../../../../assets/papers-v3/2412.12009/fig3-ablation-spiral-h.png)：原 Fig. 3，两阶段消融，LaTeX `secs/experiment.tex`，资产 `assets/fig3-ablation-spiral-h.pdf`；作者源图，未裁剪，CC BY 4.0 元数据，允许署名复用。
