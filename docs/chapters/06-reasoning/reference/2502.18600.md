...
![Comparison of Claude 3.5 Sonnet accuracy and token usage across GSM8K, Date, Sports, and Coin Flip for Standard, CoT, and CoD.](../../../../assets/papers-v3/2502.18600/fig1_plot.png)

图 1 是作者源文件中的 `plot.png`，本地复用文件为 [`assets/fig1_plot.png`](../../../../assets/papers-v3/2502.18600/fig1_plot.png)。上半图 y 轴是 Accuracy (%)，横轴依次 GSM8K、Date、Sports、Coin Flip；每组蓝/黄/红分别是 Standard/CoT/CoD。下半图 y 轴是 Token Count，展示 CoT 的柱子通常远高于 CoD，尤其 Sports。图的数值是 Claude 3.5 Sonnet 汇总，和表 1--4 的 Claude 行对应；它没有画 latency，也没有误差条。图中 Coin Flip 的 CoT 与 CoD accuracy 都在 100%，而 token 量仍有明显差距，直观显示“相同正确率、不同生成负担”。
