...
```math
x_i^{new}=\mathrm{Concat}\bigl(x_i,\;\text{“Think for }n_{gold,i}\text{ tokens.”}\bigr).
```
...
\mathrm{clip}\bigl(\alpha(n_{gold}-n_y)+\delta,0,1\bigr),
\qquad \delta=0.5.
```
...
奖励变体的算例能说明为何不能只看违约率。设 $`n_{gold}=1000`$、正确回答长度 $`n_y=1100`$，取 $`\alpha=0.003`$、$`\delta=0.5`$，则长度项为 $`\mathrm{clip}(0.003(-100)+0.5,0,1)=0.2`$，总奖励为 0.2；若回答正确且 $`n_y=700`$，长度项为 $`\mathrm{clip}(0.003(300)+0.5,0,1)=1`$，总奖励为 1；若回答错误，即使长度完美，总奖励仍为 0。这里的 $`\alpha=0.003`$ 只是解释公式的教学数值，论文没有声称采用它。论文附录 A.7 的 Addition 变体使用加法奖励 $`\mathbb{I}(y=y_{gold})-\alpha|n_{gold}-n_y|`$，确实达到 0% budget violation，但模型坍缩为极短 CoT，准确率显著下降。它是“长度验收漂亮、任务结果失败”的反例。
...
**Figure 3（原图：![OOD length-control performance](../../../assets/papers/2503.04697/figure3-ood.png)，`assets/figures/pass_rate_vs_tokens_used_ood.pdf`，CC BY 4.0）** 展示 GPQA、LSAT 和 MMLU。GPQA/LSAT 的 pass rate 随预算呈与数学任务相似的上升趋势，在可比预算接近 DeepScaleR-4K；MMLU 的线性关系更弱，作者给出 $`R^2=0.66`$，解释为知识题不太需要长推理。GPQA、LSAT 没有出现在 LCPO 数学 RL 数据中，MMLU 被认为更远离 R1-1.5B 训练分布；不过“可能在基础模型训练域内”是作者对数据暴露的判断，不是严格去污染证明。

**Figure 4 与 Figure 5（原图：![Length-control distributions](../../../assets/papers/2503.04697/figure4-length-control.png)、![L1-Max soft budget violations](../../../assets/papers/2503.04697/figure5-violations.png)，资产为 `length_error_comparison*.pdf` 与 `budget_violations_combined_soft_high.pdf`，CC BY 4.0）** 表明数学任务的平均长度误差约 3%，目标 512、1,024、2,048、3,600 都能形成相应长度分布。OOD 误差更高：正文称约 20–40%，附录 Figure 11 给出的均值约为数学任务 2.2%–2.9%、GPQA 6.6%、LSAT 20.8%、MMLU 40.5%；MMLU 在高目标长度尤明显。附录 Figure 9 显示 MMLU 的长预算误差可达约 49%，因为额外 CoT 对知识题帮助有限，模型会倾向于提前/偏离目标。附录 Table 2 的额外 500 RL steps（L1-Exact+）把数学 mean error 从 3.01% 变为 3.15%，但 RMSE 从 18.44% 降到 10.04%；OOD-1 mean error 从 21.22% 降到 10.89%，OOD-2 从 40.54% 降到 23.57%。更严格的跟长会在高预算轻微损伤准确率，说明控制精度与可为难题继续思考之间存在取舍。
...
**训练动态（附录 Figure 14，原图 ![LCPO-Exact training logs](../../../assets/papers/2503.04697/figure14-training-dynamics.png)，`assets/figures/training_logs.png`）。** 前约 300 steps，模型首先提高 solve rate，同时最短回答长度下降；约第 300 step 后，reward 和 token-adherence（reward 减 solve mean）快速上升，最短长度骤降并接近训练下限 100。作者称之为“aha moment”，即策略在训练中突然学到长度概念。它是训练日志中的相变描述，不等于普适的优化理论。
