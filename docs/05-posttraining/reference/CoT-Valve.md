...
```math
p(a\mid t_1,\ldots,t_n,q;\theta)\prod_{i=1}^{n}p(t_i\mid t_{\lt i},q;\theta).
```

若另有较短的正确思路 $`t_1,\ldots,t_m`$，其中 $`m<n`$，目标是找到参数改变量 $`\Delta\theta`$，使模型在短思路上仍能生成正确答案：

```math
\max_{\Delta\theta}\;\mathbb E_{(q,a)\sim D}\left[p(a\mid t_{1:m},q;\theta+\Delta\theta)\prod_{i=1}^{m}p(t_i\mid t_{\lt i},q;\theta+\Delta\theta)\right].
```
