...
~~~math
H(x_i)=\sum_{v\in\mathcal V}p_e(x_i=v\mid\mathbf{x}_{\lt i})\log p_e(x_i=v\mid\mathbf{x}_{\lt i}).
~~~
...
![stride、BPE、entropy、space和CNN entropy的patch分割](../../../assets/papers/2412.09871/patching-types.png)
...
~~~math
f_p(\mathbf{x}_{\lt i})=f_p(\mathbf{x})_{\lt i}.
~~~
...
e_i=x_i+\sum_{n=3}^{8}E_n^{hash}(\mathrm{Hash}(g_{i,n})).
~~~
...
P_l=P_{l-1}+W_o\mathrm{softmax}\left(\frac{QK^\top}{\sqrt{d_k}}\right)V.
~~~
...
~~~math
\mathrm{FL}_{BLT}=
\frac{\mathrm{TransfFL}(h_G,l_G,m=n_{ctx}/n_p,V=0)}{n_p}
+\mathrm{TransfFL}_E+\mathrm{TransfFL}_D
+\mathrm{CrossAttnFL}_E+\mathrm{CrossAttnFL}_D.
~~~
...
~~~math
\mathrm{BPB}(x)=\frac{\mathcal L_{CE}(\mathbf{x})}{\ln 2\cdot n_{bytes}}.
~~~
...
![space与entropy BLT的compute-optimal scaling](../../../assets/papers/2412.09871/scaling-compute-opt-part-1.png)

![space与entropy BLT的compute-optimal scaling](../../../assets/papers/2412.09871/scaling-compute-opt-part-2.png)
...
![两个inference FLOPs档的固定推理成本scaling](../../../assets/papers/2412.09871/fixed-inference-part-1.png)

![两个inference FLOPs档的固定推理成本scaling](../../../assets/papers/2412.09871/fixed-inference-part-2.png)
