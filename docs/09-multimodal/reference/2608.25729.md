...
```math
X_{out}=X+\mathrm{TTT\mbox{-}Conv}(\mathrm{RMSNorm}(X)).
```
...
```math
z_t=\mathrm{SiLU}(\mathrm{Conv2d}(q_t,W_{1,t})),
\qquad
\hat{x}_t=\mathrm{Conv2d}(z_t,W_{2,t}).
```
...
```math
\mathcal{F}_{final}=\mathrm{Sort}(S\cup U),
...
![Figure 3: Importance score components across 128 frames of a representative video.](../../../assets/papers/2608.25729/importance.png)
...
![Figure 2: System optimization results measured on A100 40GB GPUs with NVLink intra-node and InfiniBand inter-node connections.](../../../assets/papers/2608.25729/system.png)
