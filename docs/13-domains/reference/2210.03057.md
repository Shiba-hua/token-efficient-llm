...
![Correlation between language frequency and MGSM accuracy for PaLM-540B](../../../assets/papers/2210.03057/figure1-frequency-accuracy.png) Figure 1：PaLM-540B 的 MGSM accuracy 与训练语料语言频率关系。横轴是语言在预训练数据中的 token percentage（对数式跨度），纵轴 accuracy；三种颜色/条件是 Translate-to-English、题目语言 CoT、English CoT。读图重点是 URL 语言并未随频率呈简单塌缩，不能由相关图得到因果证明。源文件 `assets/figure1-frequency-accuracy.tex`。

![MGSM problem distribution with respect to number of reasoning steps](../../../assets/papers/2210.03057/figure2-mgsm-stats.png) Figure 2：250 道 MGSM 题按官方解答所需 2--8 步分布；它说明 benchmark 的任务确实包含多步算术，但不是模型输出步数统计。源文件 `assets/figure2-mgsm-stats.pdf` 与 `assets/figure2-mgsm-stats.tex`。

![The chain-of-thought prompts and example model outputs in the MGSM experiments](../../../assets/papers/2210.03057/figure3-mgsm-prompts.png) Figure 3：native exemplar prompting 下的多语 CoT 示例与模型输出，展示 Bengali 题目/推理与 multilingual exemplar 的组织方式。它解释“题目语言”和“推理语言”如何组合，不是性能图。源文件 `assets/figure3-mgsm-prompts.pdf`。

![MGSM accuracy with different model scales](../../../assets/papers/2210.03057/figure4-scaling-laws.png) Figure 4：GPT-3 系列与 PaLM 8B/62B/540B 的 MGSM accuracy 随规模变化；横向比较受模型系列、提示与闭源模型差异影响，图支持“规模增大通常提升”的观察。源文件 `assets/figure4-scaling-laws.tex`。

![MGSM accuracy of PaLM-540B with different numbers of few-shot exemplars](../../../assets/papers/2210.03057/figure5-n-shots.png) Figure 5：PaLM-540B 在 1、2、4、6 个 few-shot exemplar 下各语言 accuracy。总体趋升但不是每个语言的每个增量都单调。源文件 `assets/figure5-n-shots.tex`。

![Prompt template in the direct answer prediction setting](../../../assets/papers/2210.03057/figure6-direct-prompt.png) Figure 6：DIRECT 的德语 prompt 模板；上方是 exemplars，下方是待解题问题与答案位置。源文件 `assets/figure6-direct-prompt.pdf`。

![Prompt template in the English CoT setting](../../../assets/papers/2210.03057/figure7-encot-prompt.png) Figure 7：EN-CoT 的德语题 prompt，展示德语输入、英语逐步解答。源文件 `assets/figure7-encot-prompt.pdf`。

![Prompt template with CoT in the question language](../../../assets/papers/2210.03057/figure8-nativecot-prompt.png) Figure 8：NATIVE-CoT 的德语题 prompt，展示德语输入和德语逐步解答。源文件 `assets/figure8-nativecot-prompt.pdf`。
