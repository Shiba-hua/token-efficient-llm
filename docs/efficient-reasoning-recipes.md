...
\sum_{t=1}^{|y|}\log p_\theta(y_t\mid x,\rho,y_{<t}).
$$
...
令 $c_i\in\{0,1\}$ 为正确性、$L_i\ge0$ 为回答长度。若同题存在正确样本，记其长度均值和标准差为 $\mu_+,s_+$，$\alpha\in[0,1)$。教学实现增加明确的数值保护 $\varepsilon>0$：
...
RLOO 在组大小 $G>1$ 时使用其余回答的平均回报作为基线：
