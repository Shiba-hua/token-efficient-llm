...
![Figure 2 原图：QwQ 对 2+3 的 13 个 solution](../../../assets/papers/2412.21187/figure-2-case-overview.png) <br>
![QwQ-32B-Preview response with 13 solutions for 2+3](../../../assets/papers/2412.21187/figure-2-case-overview.png) 图 2 显示 13 个解法和传统模型答案。正文给出的可手算摘要是：Solution 1 用 39 tokens 得到正确答案；全响应 901 tokens，因此该样本的 outcome efficiency 是 $`39/901=4.3\%`$。13 个解法按策略被聚成 7 类：直接陈述算术事实（1、6、11）；拆成更小步骤（2、5）；数物体类比（3、4）；用减法反向检查（7）；代数方程验证（8）；二进制/罗马数字转换（9、10）；以及模运算或编程等特定框架（12、13）。这支持“重复解法占据大量后缀”的诊断，但不能据此断言模型内部只有 7 个思维状态。
...
![Solution count distribution and MATH500 difficulty decomposition](../../../assets/papers/2412.21187/figure-3-solution-distribution-part-1.png)

![Solution count distribution and MATH500 difficulty decomposition](../../../assets/papers/2412.21187/figure-3-solution-distribution-part-2.png)

![Solution count distribution and MATH500 difficulty decomposition](../../../assets/papers/2412.21187/figure-3-solution-distribution-part-3.png) 图 3/4 的解读重点是“轮次数量”和“token 数量”是两个轴向不同的资源信号，不能把较多轮次直接等同于较高成本或较低准确率。论文没有报告每个难度桶的完整原始数值表，正文图中可直接读取的代表值如上。
...
![QwQ and DeepSeek-R1 outcome/process efficiency across MATH500 levels](../../../assets/papers/2412.21187/figure-7-efficiency-levels-part-1.png)

![QwQ and DeepSeek-R1 outcome/process efficiency across MATH500 levels](../../../assets/papers/2412.21187/figure-7-efficiency-levels-part-2.png) 图 7 按 MATH500 难度画出 QwQ 与 R1 的 outcome/process。作者特别指出 Level 1 的 outcome efficiency 低于 50%，与 ASDIV 的低效率一致。图 7 支持“简单题更容易过度生成”的局部结论，但只覆盖两个模型、一个困难度标注体系，不能推导普遍的输入自适应规律。
...
![Illustration of overthinking: 2+3 token usage and MATH500 token-accuracy plot](../../../assets/papers/2412.21187/figure-1-token-accuracy-part-2.png) 图 1(b) 是 MATH500 token-accuracy plot，展示 QwQ 与方法输出长度和准确率关系；应读作预算/前缀效率的可视化，不是端到端 GPU 成本曲线。论文正文没有报告每个方法的 wall-clock latency、FLOPs、KV-cache、训练成本、显存或功耗。
