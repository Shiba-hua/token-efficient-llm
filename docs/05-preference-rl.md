...
\operatorname{clip}(\rho_t,1-\epsilon_{\mathrm{clip}},1+\epsilon_{\mathrm{clip}})\hat A_t\big).
$$
...
练习：把 ratio 换成 0.5，分别代入正负优势；解释为什么结果不是简单的 $\operatorname{clip}(\rho)A$。再给一个“短而错、长而对”的回答对，写出偏好规则。最后把两条回答交换，观察标签错误会如何改变更新方向。
