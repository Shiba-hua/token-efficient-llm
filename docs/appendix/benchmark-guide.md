...
本页集中保留数据审核、真实基准样例与判分说明。公开示例用于理解任务，不应拿已讲解的题当盲测集。
...
![数据审核的四个独立维度](../../assets/teaching/data-four-axes.svg)

*图 1：本页的教学材料自制概念图。每个维度都需要自己的证据；四项没有经过同标准审计时，不给数据集排“最干净”名次。图中没有真实语料的质量测量。*
...
![预算曲线可能交叉](../../assets/teaching/evaluation-frontier.png)
...
| 基准 | 任务对象 | 核心性能标准 | 原生效率口径 | 对本页的教学材料主目标的用途 |
| --- | --- | --- | --- | --- |
...
![AMO-0 三角格点](../../assets/teaching/evaluation-ock-triangle.png)
...
**复现需要选择明确口径。** 上述固定仓库的数学实现使用 LLM judge，而 `scoring.py` 从 `tokens.total_tokens` 取平均成本，和论文输出长度口径不同。不要混用。可分别报告论文口径 OckScore 和本页的教学材料完整输入+输出曲线，并保存原始账目。[数学判分源码](https://github.com/OckBench/OckBench/blob/b72752a0ba5989d3c212675ec60d3eedcd838628/src/evaluators/math_eval.py)、[聚合源码](https://github.com/OckBench/OckBench/blob/b72752a0ba5989d3c212675ec60d3eedcd838628/src/core/scoring.py)

### OptimalThinkingBench：别在简单题浪费，也别在难题草率
...
![OTB 官方迷宫实例的代码重绘](../../assets/teaching/evaluation-otb-maze.png)

黑格是墙，白格可通行，S/G 对应原题起止符号；橙色路径是本页的教学材料用广度优先搜索补充的解，15 步已与参考值核对。只改了视觉表示，未另造一个迷宫。
...
![AUCOAA 与聚合顺序](../../assets/teaching/evaluation-aucoaa.png)
...
![同长度下的效率比率反例](../../assets/teaching/evaluation-think-ratio.png)
...
![AppWorld 原始框架图](../../assets/papers/2407.18901/appworld-main.png)

图由 Harsh Trivedi 等作者发布，来自 LaTeX 源工程 `images/main.pdf`；[原始PDF资产](../../assets/papers/2407.18901/appworld-main.pdf)字节未改。这里只将单独图像PDF转成PNG供GitHub显示，未截取论文页面。许可 [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)，来源与校验和见[记录](../../meta/original-figure-provenance.json)。左侧是模拟应用，中间是代码—执行反馈循环，右侧比较初态与终态。

以下三个例子都来自作者公开图表中的真实场景。它们是公开场景级示例；具体实例的人名、日期和数据库值由任务变体给出，本页的教学材料不补造这些参数。
...
一次评测可以采用这样的记录结构。它是本页的教学材料建议的日志格式，不冒充任何官方格式：
...
5. AppWorld 三个变体只传入两条成功记录会怎样？本页的教学材料实现拒绝聚合；正式评测也必须检查完整场景覆盖。

[返回目录](../../README.md) · [上一章：智能体编排](worked-examples.md) · [下一章：相邻方向](worked-examples.md)
