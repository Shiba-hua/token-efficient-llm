...
\mathrm{softmax}\bigl(f_u(f_{h_i}(f_s(x_{t:1})))\bigr),\quad i=1,\ldots,n.
```
...
- `figure-1`: 总览：训练时共享 trunk + 4 heads，推理时只用 next-token head 或启用额外 heads；下方为 MBPP 随模型规模的 pass@1。来源同上；CC BY 4.0；`reuse_allowed: true`；![Overview of multi-token prediction and MBPP scaling trend (TeX label fig:main_figure_col; img/main_fig_col.png)](../../../assets/papers/2404.19737/figure-1.png)
- `figure-2`: $`n=2`$ 时逐 head forward/backward 的顺序与释放 logits 梯度的内存机制；来源同上；CC BY 4.0；![Sequential forward/backward over prediction heads for memory efficiency (TeX label fig:backward; img/backward_order.png)](../../../assets/papers/2404.19737/figure-2.png)
- `figure-3`: 0.3B–13B 代码模型在 MBPP/HumanEval 的 pass@1/10/100，显示小模型退化、规模上升后反转；来源同上；CC BY 4.0；![Scaling results on MBPP and HumanEval (TeX label fig:scaling_model; img/scaling.pdf)](../../../assets/papers/2404.19737/figure-3.png)
- `figure-4`: CodeContests 微调，比较 $`n'=1`$ / $`n'=4`$ 及 next-token 预训练基线，横轴 pass@k、温度取 oracle；来源同上；CC BY 4.0；![CodeContests finetuning comparison (TeX label fig:dm_contests; img/codecontests.pdf)](../../../assets/papers/2404.19737/figure-4.png)
- `figure-5`: 7B、200B natural-language tokens 的 6 个选择/似然 benchmark 平均准确率，$`n=2`$ 近似基线、$`n=4`$ 退化；来源同上；CC BY 4.0；![Natural-language choice-task average accuracy (TeX label fig:nlp_evol; img/dill_evals_choice.pdf)](../../../assets/papers/2404.19737/figure-5.png)
- `figure-6`: 8 个摘要 benchmark 的平均 ROUGE-L $`F_1`$，200B/500B 训练数据，$`n=2,4`$ 多数提升但个别数据集反例；来源同上；CC BY 4.0；![Abstractive summarization ROUGE-L (TeX figure input summarize_scale; img/summarization_valid.pdf)](../../../assets/papers/2404.19737/figure-6.png)
- `figure-7`: 1M–1B 模型归纳能力，30M 以下 2-token 更强，100M 后差异消失；来源同上；CC BY 4.0；![Induction capability by model size (TeX label fig:induction_size; img/induction.pdf)](../../../assets/papers/2404.19737/figure-7.png)
- `figure-8`: 多项式算术的域内/域外泛化，multi-token 跨难度提升；来源同上；CC BY 4.0；![Polynomial arithmetic in-domain and OOD accuracy (TeX label fig:poly_0; img/poly_100M_pause_0.pdf)](../../../assets/papers/2404.19737/figure-8.png)
- `figure-9`: consequential choice point 的隐式加权示例；来源同上；CC BY 4.0；![Lookahead reinforces consequential choice points (TeX label fig:weights; img/implicit_weights.png)](../../../assets/papers/2404.19737/figure-9.png)
- `figure-S10`: self-speculative 相对速度/延迟随 head 数，见附录 A；![Self-speculative decoding relative speed and latency (TeX label fig:decoding; composite of img/throughput.pdf and img/latency.pdf; no single source image)](../../../assets/papers/2404.19737/figure-S10-part-1.png)

![Self-speculative decoding relative speed and latency (TeX label fig:decoding; composite of img/throughput.pdf and img/latency.pdf; no single source image)](../../../assets/papers/2404.19737/figure-S10-part-2.png)
- `figure-S13`: GSM8K 200B/500B 的采样 pass@k，展示数据规模反转与 $`n=4`$ 退化；![GSM8K results at 200B and 500B natural-language training (TeX label fig:gsm8k; img/gsm8k.pdf)](../../../assets/papers/2404.19737/figure-S13.png)
