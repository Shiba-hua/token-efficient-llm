...
```math
\mathcal{L}_{\mathrm{ref}}(x_i)=-\log P_{\mathrm{RM}}(x_i\mid x_{\lt i}).
```
...
-\frac{1}{N\,k\%}\sum_{i=1}^{N}I_{k\%}(x_i)\log P_{\theta}(x_i\mid x_{\lt i}).
```
...
- ![Math continual pretraining accuracy curves; SLM reaches CLM baseline about 5-10x faster](../../../assets/papers/2404.07965/fig1.png) 图 1：数学持续预训练时 GSM8K/MATH 平均 few-shot accuracy 与训练进度，SLM 达到 CLM baseline 约快 5–10×。来源为 v1 HTML/PDF；原图 TeX/图片资产在当前网络环境未能下载，故 `local_file` 缺失。
- ![Token noise after document filtering; CLM versus SLM loss mask](../../../assets/papers/2404.07965/fig2.png) 图 2：上部说明文档级过滤后仍有 token 噪声；左 CLM 对所有 token 算 loss，右 SLM 仅对 useful/clean token 算 loss。方法概念图，不应误读为删除上下文。
- ![Four token loss trajectory categories and fluctuations](../../../assets/papers/2404.07965/fig3.png) 图 3：四类 token loss trajectory 及 L→L/H→H 的波动示例；约 26% H→L、51% L→L、11% H→H、12% L→H。
- ![Three-step SLM pipeline](../../../assets/papers/2404.07965/fig4.png) 图 4：RM 训练→逐 token RM scoring→按 excess loss 选择→SLM 预训练的三步流程。
- ![General pretraining results on 80B tokens](../../../assets/papers/2404.07965/fig5.png) 图 5：TinyLlama-1B 在 80B general tokens 上的 15-task 结果，平均提升 6.8%，code/math 超过 10%。
- ![Pretraining selected/unselected loss and downstream loss](../../../assets/papers/2404.07965/fig6.png) 图 6：selected/unselected token 的训练 loss 与 downstream loss（约 4B 预训练 token）。
- ![Selected/unselected loss versus downstream accuracy](../../../assets/papers/2404.07965/fig7.png) 图 7：selected/unselected token loss 与 GSM8K+MATH accuracy 的幂律关系。
- ![PPL of tokens selected at different checkpoints](../../../assets/papers/2404.07965/fig8.png) 图 8：不同 checkpoint 选中 token 的 PPL 与动态选择现象。
- ![Effect of token selection ratio](../../../assets/papers/2404.07965/fig9.png) 图 9：token selection ratio 消融，约 60% 合适。
